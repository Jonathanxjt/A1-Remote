...
const EmployeeStatusPieChart: React.FC<EmployeeStatusPieChartProps> =
  React.memo(({ employees }) => {
    ...
    const wfhCount = employees.filter((employee) =>
      ["AM", "PM", "Full"].includes(employee.status)
    ).length;
    const inOfficeCount = employees.filter(
    ...
    const options = {
      ...
      exportMenu: {
        visible: false, // Disable the export menu to remove the three-dot option
      },
    };
