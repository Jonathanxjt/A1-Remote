    ...
    const markAsRead = async (ids: number[]) => {
      try {
        await Promise.all(
          ids.map((id) =>
            axios.put(
              `${api.NOTIFICATION_URL}/notification/read_notification/${id}`
            )
          )
        );
    ...
    const deleteNotifications = async (ids: number[]) => {
      try {
        await Promise.all(
          ids.map((id) =>
            axios.delete(
              `${api.NOTIFICATION_URL}/notification/delete_notification/${id}`
            )
          )
        );
      ...
      const updatedNotifications = notifications.filter(
        (notification) => !ids.includes(notification.notification_id)
      );
      setLocalNotifications(updatedNotifications);
      setNotifications(updatedNotifications);  // Update parent state
